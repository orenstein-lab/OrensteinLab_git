...
def motor_scan(map_dict, inst_dict={}, mobj_dict={}, iobj_dict={}, filename_head=None, filename=None, savefile=True, print_flag=False, vars=[], plot=True, metadata={}, close_devices=True):
    ...
    instruments_header = helper.get_instruments_header(iobj_dict, ikwargs_dict)
    ...
    metadata = {**metadata, **read_motors(ACTIVE_MOTORS, mobj_dict)}
    ...
    helper.move_motors_to_start(motors, mobj_dict, mkwargs_dict, positions, print_flag=print_flag)
    ...
    for ii in tqdm(range(num_pos)):
        ...
        helper.move_motors(motors, mobj_dict, mkwargs_dict, current_pos, start_pos, pos, print_flag=print_flag)
        current_pos = pos
...
def motor_scan_balance(map_dict, balance_dict, inst_dict={}, mobj_dict={}, iobj_dict={}, filename_head=None, filename=None, savefile=True, print_flag=False, vars=[], plot=True, metadata={}, close_devices=True):
    '''
    method to record measurements on instruments as a function of motors specified by dictionary map_dict. meant to acquire data one point at a time in a multidimensional motor space.
...
def timed_measurement(recording_time, inst_dict={}, mobj_dict={}, iobj_dict={}, filename_head=None, filename=None, savefile=True, plot=True, vars=[], metadata={}, close_devices=True):
    '''
    ...
    if savefile:
        filename_head, filename = helper.generate_filename(filename_head, filename, 'timed_measurement')
        fname = helper.get_unique_filename(filename_head, filename)
        helper.write_file_header(fname, header, metadata)
...
def motor_sequence(sequence_list, inst_dict={}, mobj_dict={}, iobj_dict={}, filename_head=None, filename=None, savefile=True, plot=True, vars=[], metadata={}, close_devices=True):
    '''
    ...
    args:
        - sequence_list:    list of motor steps to make sequentially, where each entry is a tuple ('motor_name', target, kwargs_dictionary, tolerance)
        - inst_dict:        dictionary of key value pairs where keys are name of instruments in INSTRUMENT_DICT and values are dictionary of kwargs for the instrument read function. Defaults to instruments in ACTIVE_INSTRUMENTS with no kwargs.
    ...
    # unpack sequence_list and check that motors in ACTIVE_MOTORS
    # sequence_motors: list of motor to move at each step
    # mtargets: list of targets for motor at each step
    # mkwargs: list of kwargs for motor at each step
    # motors: list of unique motors involved in sweep
    # tols: list of tolerances for stability conditon for each step
    sequence_motors, mtargets, mkwargs, motors, tols = helper.capture_sequence_information(sequence_list)
    for m in motors:
        if m not in ACTIVE_MOTORS:
            raise ValueError(f'motor {m} not in ACTIVE_MOTORS {ACTIVE_MOTORS}')
    ...
    header = ['Time (s)']+instruments_header+motors_header
    if set(vars)!=set(header):
        raise ValueError(f'plotvars {vars} not in measurd values {header}')
    if savefile:
        filename_head, filename = helper.generate_filename(filename_head, filename, 'motor_sweep')
        fname = helper.get_unique_filename(filename_head, filename)
        helper.write_file_header(fname, header, metadata)
...
def monitor_motors(motors, filename_head=None, filename=None, savefile=False, plot=True):
    '''
    Utility to measure the value of a motor as function of time, with a quick way of exiting.

    args:
        - motors:           list of motors to display. single motor will be turned into a list.
        - filename_head:
        - filename:
        - savefile:
        - plot
...
def rotate_scan(start_angle, end_angle, step_size, axis_index=1, inst_dict={}, mobj_dict={}, iobj_dict={}, filename_head=None, filename=None, savefile=True, print_flag=False, vars=[], plot=True, metadata={}, close_devices=True):
    '''
    ...
    # 
    # automatically writes a file to filename_head/autogenerate if savefile=True and filename=None
    header = [f'Axis {axis_index} (deg)']+instruments_header+motors_header
    if set(vars)!=set(header):
        raise ValueError(f'vars {vars} not in measurd values {header}')
    if savefile:
        filename_head, filename = helper.generate_filename(filename_head, filename, 'rotate_scan')
        fname = helper.get_unique_filename(filename_head, filename)
        helper.write_file_header(fname, header, metadata)
    ...
    # move axis to start
    move_axis(angle-move_back, axis=axis_obj)
    move_axis(angle, axis=axis_obj)
    time.sleep(0.1)
    ...
    move_axis(start_angle, axis=axis_obj)
...
def corotate_scan(start_angle, end_angle, step_size, angle_offset, rate_axis_2=1, inst_dict={}, mobj_dict={}, iobj_dict={}, filename_head=None, filename=None, savefile=True, print_flag=False, vars=[], plot=True, metadata={}, close_devices=True):
    '''
    ...
    # 
    # automatically writes a file to filename_head/autogenerate if savefile=True and filename=None
    header = ['Axis 1 (deg)', 'Axis 2 (deg)']+instruments_header+motors_header
    if set(vars)!=set(header):
        raise ValueError(f'vars {vars} not in measurd values {header}')
    if savefile:
        filename_head, filename = helper.generate_filename(filename_head, filename, 'corotate_scan')
        fname = helper.get_unique_filename(filename_head, filename)
        helper.write_file_header(fname, header, metadata)
    ...
    # move axis to start
    newport.corotate_axes(1, 2, start_angle-move_back_1, start_angle+offset_angle-move_back_2, axis_1=axis_1, axis_2=axis_2)
...
def rotate_map(map_dict, start_angle, end_angle, step_size, channel_index=1, inst_dict={}, mobj_dict={}, iobj_dict={}, filename_head=None, filename=None, savefile=True, print_flag=False, vars=[], plot=False, metadata={}, close_devices=True):
    '''
    ...
    args:
        ...
        - end_angle:        
        - step_size:        
        - channel_index:    which index to rotate
    ...
    for ii in tqdm(range(num_pos)):
        ...
        # scan
        rotate_scan(start_angle, end_angle, step_size, axis_index=1, inst_dict=inst_dict, mobj_dict=mobj_dict, iobj_dict=iobj_dict, filename_head=filename_head, filename=filename, savefile=True, print_flag=print_flag, vars=[], plot=plot, metadata=metadata, close_devices=False)
...
def corotate_map(map_dict, start_angle, end_angle, step_size, angle_offset, rate_axis_2=1, inst_dict={}, mobj_dict={}, iobj_dict={}, filename_head=None, filename=None, savefile=True, print_flag=False, vars=[], plot=True, metadata={}, close_devices=True):
    '''
    ...
    for ii in tqdm(range(num_pos)):
        ...
        # scan
        corotate_scan(start_angle, end_angle, step_size, angle_offset, rate_axis_2=rate_axis_2, inst_dict=inst_dict, mobj_dict=mobj_dict, iobj_dict=iobj_dict, filename_head=filename_head, filename=filename, savefile=True, print_flag=print_flag, vars=[], plot=plot, metadata=metadata, close_devices=False)
...
def find_balance_angle(start_angle, end_angle, step_size, balance_at=0, offset=0, bal_axis=2, go_to_balance_angle=True, var='Demod 1 x', inst_dict={}, mobj_dict={}, iobj_dict={}):
    '''
    ...
    move_static_axis(balance_at-move_back_static_axis, static_axis_obj)
    move_bal_axis(balance_at-move_back_bal_axis, bal_axis_obj)
    move_static_axis(balance_at, static_axis_obj)
    move_bal_axis(balance_at, bal_axis_obj)
    ...
    for ii, angle in enumerate(angles):
        ...
        # move
        move_bal_axis(angle, bal_axis_obj)
    ...
    if go_to_balance_angle:
        move_bal_axis(balance_angle+balance_at, bal_axis_obj)
...
def autobalance(slope, tolerance, offset=0, bal_axis=2, go_to_balance_angle=True, var='Demod 1 x', inst_dict={}, mobj_dict={}, iobj_dict={}, print_flag=True):
    ...
    args:
        ...
        - tol                   tolerance for minizing signal. when signal below tol threshold, functions return.
        - offset:               target balanced signal
        - bal_axis:             axis with which to balance
        - go_to_balance_angle:  if true, set axis to balance angle
        - var:                  variable signal to monitor
    ...
    balance_at = read_static_axis(bal_static_obj)

    # minimize var-offset
    signal = 10000
    curr_pos = read_bal_axis(bal_axis_obj)
    while (np.abs(signal-offset)>tolerance):
        

        signal = helper.read_instruments(iobj_dict, ikwargs_dict)[var]
        new_pos = (curr_pos-(1/slope)*(signal-offset))%360
        move_bal_axis(new_pos, bal_axis_obj)
        curr_pos = new_pos
