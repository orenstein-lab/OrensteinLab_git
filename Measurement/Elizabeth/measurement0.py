...
def rotate_scan(start_angle, end_angle, step_size, filename_head=None, filename=None, axis_index=2, measure_motors=[], showplot=True, time_constant=0.3, channel_index=1, R_channel_index=2, daq_objs=None, axis_1=None, axis_2=None):
...
def corotate_scan(start_angle, end_angle, step_size, angle_offset, filename_head=None, filename=None, measure_motors=[], showplot=True, time_constant=0.3, channel_index=1, R_channel_index=2, daq_objs=None, axis_1=None, axis_2=None, rate_axis_2=1):
    '''
...
def corotate_map_ct(map_dict, start_angle, end_angle, step_size, angle_offset, rate_axis_2=1, filename_head=None, filename=None, measure_motors=[], showplot=False, time_constant=0.3, channel_index=1, R_channel_index=2, print_flag=False):
    '''
    Takes a corotation scan at each point in a map specified by dictionary map_dict, which entries of the form 'axis':(start, end, step_size, kwargs), where kwargs is a dictionary of key/value pairs appropriate for each motor 'move' function. For example, a temperature map might take the following map dictionary:
    ...
    for ii in tqdm(range(num_pos)):
        ...
        # COIL MODULATION
        daq.setInt('/dev3537/triggers/out/0/source', 0)
        daq.setInt('/dev3537/demods/0/oscselect', 2)
        daq.setDouble('/dev3537/oscs/2/freq', 1000)
        daq.setInt('/dev3537/sigouts/0/enables/0', 1)
        daq.setInt('/dev3537/sigouts/0/on', 1)
        time.sleep(10)
        daq.setDouble('/dev3537/demods/0/phaseshift', 0)
        time.sleep(1)
        # setup each filename
        expanded_filename = filename
        for ii, m in enumerate(motors):
            ...
            if m == 'temp':
                p = round(p,5)
            expanded_filename = expanded_filename+f'_coil_{m}{p}'

        # scan
        corotate_scan(start_angle, end_angle, step_size, angle_offset, rate_axis_2=rate_axis_2, filename_head=filename_head, filename=expanded_filename, measure_motors=measure_motors, showplot=showplot, time_constant=time_constant, channel_index=channel_index, R_channel_index=R_channel_index, daq_objs=daq_objs, axis_1=axis_1, axis_2=axis_2)

        # THERMAL MODULATION
        daq.setInt('/dev3537/sigouts/0/on', 0)
        daq.setInt('/dev3537/sigouts/0/enables/0', 0)
        daq.setInt('/dev3537/demods/0/oscselect', 0)
        daq.setInt('/dev3537/triggers/out/0/source', 1)
        time.sleep(10)
        daq.setInt('/dev3537/demods/0/phaseadjust', 1)
        time.sleep(1)
        # setup each filename
        expanded_filename = filename
        for ii, m in enumerate(motors):
            ...
            if m == 'temp':
                p = round(p,5)
            expanded_filename = expanded_filename+f'_thermal_{m}{p}'

        # scan
        corotate_scan(start_angle, end_angle, step_size, angle_offset, rate_axis_2=rate_axis_2, filename_head=filename_head, filename=expanded_filename, measure_motors=measure_motors, showplot=showplot, time_constant=1, channel_index=channel_index, R_channel_index=R_channel_index, daq_objs=daq_objs, axis_1=axis_1, axis_2=axis_2)
        current_pos = pos
