...
def Pump_probe_tempdep(start_temp, end_temp, step_size, channel_index, R_channel_index, time_constant, showplot, filename_head, filename):
    # Lock-in Amplifier initialization
    apilevel = 6
    (daq, device, props) = ziutils.create_api_session(device_id, apilevel)
...
def Pump_probe_corotate_tempdep(axis_index_1, start_pos_1, end_pos_1, step_size_1, go_back_1, axis_index_2, start_pos_2, end_pos_2, step_size_2, go_back_2, start_temp, end_temp, temp_step_size, channel_index, R_channel_index, time_constant, showplot, filename_head, filename):
    ...
    for temp in Trange:
        ...
        for i in np.arange(0,len(scan_range_1),1):
            ...
            file.write(format(angle_1[-1],'.15f')+"\t"+format(angle_2[-1],'.15f')+"\t"+format(x,'.15f')+'\t'+format(y,'.15f')+'\t'+format(r,'.15f')+'\n')
            file.close()
...
def Pump_probe_corotate_WP_tempdep(axis_index_1, axis_index_2, start_angle, end_angle, angle_step_size, go_back, offset, start_temp, end_temp, temp_step_size, channel_index, time_constant, showplot, filename_head, filename):
    # ESP301 initialization
    controller = newport.NewportESP301.open_serial(port=port_id, baud=921600)
    ...
    scan_range_1 = get_angle_values(start_angle, end_angle, angle_step_size)
    scan_range_2 = get_angle_values(start_angle, end_angle, angle_step_size) + offset
    ...
    for temp in Trange:
        ...
        for i in np.arange(0,len(scan_range_1),1):
            ...
            file.write(format(WP1_angle[-1],'.15f')+"\t"+format(WP2_angle[-1],'.15f')+"\t"+format(x,'.15f')+'\t'+format(y,'.15f')+'\t'+format(r,'.15f')+'\n')
            file.close()
...
def Mapping_quicksave(num_of_steps, axis_index_1, x_start_pos, x_step_size, x_num_steps, x_tol, axis_index_2, y_start_pos, y_step_size, y_num_steps, y_tol, go_back, channel_index, R_channel_index, time_constant, showplot, filename_head, filename):
    #Lock-in Amplifier initialization
    apilevel = 6
    (daq, device, props) = ziutils.create_api_session(device_id, apilevel)
    ...
    for y_pos in y_range:
        for x_pos in x_range:
            ...
            for i in np.arange(0,len(scan_range_1),1):
                ...
                #Save file
                file = open(filename,'a')
                file.write(format(float(position[-1]), '.15f')+"\t"+format(float(position[-1]), '.15f')+"\t"+format(x, '.15f')+'\t'+format(y, '.15f')+'\t'+format(r, '.15f')+'\n')
                file.close()
