...
def Pump_probe_tempdep_quick(start_temp, end_temp, step_size, channel_index, time_constant):
    # Lock-in Amplifier initialization
    apilevel = 6
    (daq, device, props) = ziutils.create_api_session(device_id, apilevel)
...
def Pump_probe_coil_therm_corotate_WP_tempdep(axis_index_1, axis_index_2, start_angle, end_angle, angle_step_size, go_back, offset, start_temp, end_temp, temp_step_size, channel_index, time_constant, chopper_phase, chopper_freq, coil_freq, showplot, filename_head, filename):
    # ESP301 initialization
    controller = newport.NewportESP301.open_serial(port=port_id, baud=921600)
    ...
    for temp in Trange:
        ...
        for i in np.arange(0,len(scan_range_1),1):
            ...
            #Plot
            if showplot == True:
                draw_x.set_data(WP1_angle_therm,demod_x_therm)
                draw_y.set_data(WP1_angle_therm,demod_y_therm)
                draw_r.set_data(WP1_angle_therm,demod_r_therm)
                ax1.relim()
                ax1.autoscale()
                ax2.relim()
                ax2.autoscale()
        ...
        for i in np.arange(0,len(scan_range_1),1):
            ...
            if showplot == True:
                draw_x.set_data(WP1_angle_therm,demod_x_therm)
                draw_x.set_data(WP1_angle_coil,demod_x_coil)
                draw_y.set_data(WP1_angle_therm,demod_y_therm)
                draw_y.set_data(WP1_angle_coil,demod_y_coil)
                draw_r.set_data(WP1_angle_therm,demod_r_therm)
                draw_r.set_data(WP1_angle_coil,demod_r_coil)
                ax1.relim()
                ax1.autoscale()
                ax2.relim()
                ax2.autoscale()
